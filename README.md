...
[![Download](https://img.shields.io/badge/⬇%20下载%20Download-v1.2.5-3b82f6?style=for-the-badge)](https://gitee.com/peng-minghang/hypo-mux-plus/releases/download/v1.2.5/HypoMuxPlus.exe)
...
- 直接下载：**[HypoMuxPlus.exe (v1.2.5)](https://gitee.com/peng-minghang/hypo-mux-plus/releases/download/v1.2.5/HypoMuxPlus.exe)**（Gitee 国内高速下载；海外用户可使用 [GitHub Releases](https://github.com/pmh1314520/HypoMuxPlus/releases/download/v1.2.5/HypoMuxPlus.exe)）
...
- **应用分流规则**：按域名 / 端口 / 进程名自定义规则——直连（不走代理）、走聚合、或钉死到指定网卡；「按进程名」匹配（如 `steam.exe`）优先级高于域名规则；支持直连白名单，以及从 URL 订阅规则列表。
- **SOCKS5 UDP 转发**：支持 SOCKS5 UDP ASSOCIATE，为代理模式下的应用经所选网卡出口转发 UDP 数据报，兼容依赖 UDP 的客户端。
- **上游代理链（多节点聚合）**：可为每张参与聚合的网卡各绑定一个上游代理节点（SOCKS5 / HTTP，支持用户名密码认证），实现「网卡 A → 节点 1」「网卡 B → 节点 2」并行叠加，同时突破本地上行带宽与单一节点两个瓶颈；支持一张网卡绑定多个上游轮转，以及全部上游不可用时的回退策略（回退直连 / 返回失败）。默认关闭，未启用时行为与既有完全一致。
- **上游健康探测与故障熔断**：周期性探测被引用上游节点的可用性与延迟，连续失败达阈值即自动熔断（暂时排除出候选），经冷却期后半开探测、成功即自动恢复；一张网卡绑定多个上游时按健康状态与延迟加权，优选更快更稳的节点。探测间隔 / 超时 / 熔断阈值 / 冷却时间均可配置。
- **订阅导入**：粘贴 Clash 订阅 / base64 订阅正文 / 节点分享链接，即可解析出受支持（socks5 / http）的上游候选，一键测速排序后确认并入上游节点列表（遵守 128 条上限）。
- **IPv6 / 双栈分流**：支持 IPv6 字面目标与域名双栈（A/AAAA）解析，并按 IP 版本偏好做 Happy-Eyeballs 式拨号与回退。
- **每网卡独立 DNS / DoH**：可为每张参与聚合的网卡单独指定明文 DNS 服务器或 DoH 端点，解析请求经该网卡出口发出，避免解析走错出口；未配置时沿用既有全局解析路径。
...
### 体检与监控
- **链路体检与测速**：一键探测各网卡出口延迟（RTT），并统计抖动（Jitter）与丢包率、展示历史趋势曲线，同时支持逐张网卡下载测速跑分，帮你挑选最健康、最快的线路；体检结果可一键导出为图片或文本报告。
...
- **系统代理防泄漏看门狗**：持续监视系统代理设置，一旦被外部程序篡改或清除即自动纠正；接管前会快照原始设置，异常退出后于下次启动补偿还原，降低流量绕过分流引擎泄漏与代理残留断网的风险。
- **稳定性加固（连接级隔离 + 上限保护）**：单条连接的异常被捕获并隔离，仅释放该连接、不波及引擎与其他连接；可配置并发连接数与后台任务并发上限，防止极端负载下资源耗尽；未处理异常写入结构化崩溃日志（含敏感信息脱敏）。
...
- **分流决策模拟器**：输入目标域名 / 进程名（可选端口），即可以纯计算方式模拟其将命中的分流路径（命中 bypass / 规则 / 承载网卡 / 直连或经上游 / 选中的上游节点），仅做展示，不发起任何真实连接、不改变加速状态，便于排查规则配置。
...
加速时程序向 `HKCU\Software\Microsoft\Windows\CurrentVersion\Internet Settings` 写入全覆盖代理链条；分流引擎收到客户端 TCP 连接后，先经所选物理网卡解析目标真实 IP（DoH/UDP，绕过 fake-ip 劫持），再用 `setsockopt(IPPROTO_IP, IP_UNICAST_IF, htonl(if_index))` 指定该连接的出口接口、`bind(local_ip, 0)` 固定源地址，使连接从目标网卡出网。多线程下载的大量并发连接被分发到不同网卡，从而在**多连接负载均衡**的意义上叠加各线路带宽。启用上游代理链时，连接会先经所选网卡的物理出口连上对应的上游节点，再由上游转发到目标。
